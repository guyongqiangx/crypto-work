...
#if 0
#define SHA3_BLOCK_SIZE		64	/* 512 bits = 64 Bytes */
#define SHA3_LEN_SIZE	 	8	/* 64 bits = 8 bytes */
#define SHA3_LEN_OFFSET 	(SHA3_BLOCK_SIZE - SHA3_LEN_SIZE)
#define SHA3_DIGEST_SIZE	20 /* 160 bits = 20 bytes */
...
typedef uint32_t (*sha3_func)(uint32_t x, uint32_t y, uint32_t z);
...
static struct sha3_context *get_sha3_context(void)
{
	static struct sha3_context _sha3;
...
int sha3_init(void)
{
	struct sha3_context *context;

	context = get_sha3_context();

    /* reset all fields to 0 */
	memset(context, 0, sizeof(struct sha3_context));
...
static int sha3_process_block(const void *block)
{
	uint32_t t;
...
int sha3_update(const void *data, uint64_t size)
{
	...
	if (context->last.size != 0)
	{
		...
		else /* more than 1 block */
		{
			...
			sha3_process_block(&context->last.buf);
