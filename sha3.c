...
static int SHA3_PrepareScheduleWord(const void *block, uint64_t *W)
{
	uint32_t t;
...
static int SHA3_UpdateTotal(uint128_t *x, uint64_t len)
{
	uint64_t l;
...
#if 0
static int SHA3_SaveTotal(uint8_t *buffer, uint128_t *len)
{
	int i;
...
static int SHA3_ProcessBlock(SHA3_CTX *ctx, const void *block)
{
	...
	SHA3_PrepareScheduleWord(block, W);
...
int SHA3_Update(SHA3_CTX *c, const void *data, size_t len)
{
	...
	if (c->last.used != 0)
	{
		...
		else /* more than 1 block */
		{
			...
			SHA3_UpdateTotal(&c->total, HASH_BLOCK_SIZE);
...
int SHA3_Final(unsigned char *md, SHA3_CTX *c)
{
	...
	{
        SHA3_UpdateTotal(&c->total, c->last.used);
        ...
        //SHA3_SaveTotal(&c->last.buf[HASH_LEN_OFFSET], &c->total);
