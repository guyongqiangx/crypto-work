...
#define SHA512_BLOCK_SIZE		64	/* 512 bits = 64 Bytes */
#define SHA512_LEN_SIZE	 	8	/* 64 bits = 8 bytes */
#define SHA512_LEN_OFFSET 	(SHA512_BLOCK_SIZE - SHA512_LEN_SIZE)
#define SHA512_DIGEST_SIZE	20 /* 160 bits = 20 bytes */

#define SHA512_PADDING_PATTERN 	0x80
#define SHA512_ROUND_NUM			80

typedef uint32_t (*sha512_func)(uint32_t x, uint32_t y, uint32_t z);
...
int sha512_final(uint8_t *hash)
{
	...
	DBG("%08x %08x %08x %08x %08x\n", context->a, context->b, context->c, context->d, context->e);
	//snprintf(hash, SHA512_DIGEST_SIZE, "%08x%08x%08x%08x%08x", context->a, context->b, context->c, context->d, context->e);
	buf = (uint32_t *)hash;
